...
#return if options has collision import off or layer name isn't "Collisions"
	if tilemap_data.__identifier == "Collisions":
		var Collisions = options.Import_Collisions and tilemap_data.__identifier == "Collisions"
		if not Collisions:
			return
...
#return if IntGrid layer is empty
	...
	layer.name = tilemap_data.__identifier
